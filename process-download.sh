...
filebot -script fn:amc --output "$final_output_dir" --action move --conflict override -non-strict --log-file "$log_file" --def unsorted=y plex=$PLEX_IP:$PLEX_TOKEN excludeList=".excludes" ut_dir="$tmp_output_dir" ut_kind="multi" ut_title="$torrent_name" ut_label=""
